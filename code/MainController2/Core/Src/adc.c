...
void MX_ADC1_Init(void)
{
  ...
  */
  ...
  // HAL_ADCEx_EnableTempSensor();     // 启用温度传感器
...
void Z_ADC_DMA_Start(ADC_HandleTypeDef *hadc)
{
  if (!adc_busy)
  {
    HAL_ADC_Start_DMA(hadc,(uint32_t *) adc_values, ADC_N_CHANNEL);
    adc_busy = 1;
  }
}
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
  ...
  float TEMP130 = (*TEMP130_CAL_ADDR) *3 / vref;
  if (hadc == &hadc1)
  {
    adc_busy = 0;
    ...
    float_to_str_1int_3frac(BAT_V, tmp);
    batnum_elm_char16.user_data = (uint8_t *) tmp;
